...
void AAuraCharacter::LoadProgress()
{
	if (AAuraGameModeBase* AuraGameMode = Cast<AAuraGameModeBase>(UGameplayStatics::GetGameMode(this)))
	{
		...
		if (AAuraPlayerState* AuraPlayerState = Cast<AAuraPlayerState>(GetPlayerState()))
		{
			...
			AuraPlayerState->SetToAttributePoints(SaveData->AttributePoints);
			AuraPlayerState->SetToSpellPoints(SaveData->SpellPoints);
		}
...
void AAuraCharacter::MulticastLevelUpParticles_Implementation() const
{
	...
	{
		const FVector CameraLocation = MainCamera->GetComponentLocation();
...
int32 AAuraCharacter::FindLevelForXP_Implementation(int32 InXP) const
{
	const AAuraPlayerState* AuraPlayerState = GetPlayerState<AAuraPlayerState>();
	check(AuraPlayerState);
	return AuraPlayerState->LevelUpInfo->FindLevelForExp(InXP);
}
...
void AAuraCharacter::ShowMagicCircle_Implementation(UMaterialInterface* DecalMat)
{
	if (AAuraPlayerController* AuraPlayerController = Cast<AAuraPlayerController>(GetController()))
	{
		AuraPlayerController->ShowMagicCircle(DecalMat);
